...
namespace Utils.Extensions
{
    public static class LogExtensions
    {
        ...
        public static string Cut(this object value, int length)
        {
            var result = string.Empty;
            var stringValue = value.ToString();

            for (int i = 0; i < length; i++)
                result += i < stringValue.Length ? stringValue[i] : '_';

            return result;
        }
        public static string Remove(this object value, string textToRemove)
        {
            var stringValue = value.ToString();
            return stringValue.Replace(textToRemove, "");
        }
